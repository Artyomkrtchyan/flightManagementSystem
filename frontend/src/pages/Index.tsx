...
const AdminPanel = ({ onBack }: { onBack: () => void }) => {
  ...
  try {
    const response = await fetch(
      `${API_URL}/api/delete?table=${activeTable}&col=${idKey}&id=${row[idKey]}`,
      { method: "DELETE" }
    );
...
const deleteSelected = async () => {
  ...
  try {
    const promises = selectedIds.map(id => 
      fetch(`${API_URL}/api/delete?table=${activeTable}&col=${idKey}&id=${id}`, { method: "DELETE" })
    );
...
return (
  ...
  const findPath = async (fromId: number, toId: number) => {
    ...
    try {
      const response = await fetch(`${API_URL}/${apiType}?id=${fromId}&to=${toId}`);
...
const findReachable = async (fromId: number) => {
  try {
    const response = await fetch(`${API_URL}/bfs?id=${fromId}&k=${Math.max(1, kValue)}`);
    const data = await response.json();
...
const findMST = async () => {
  try {
    const response = await fetch(`${API_URL}/mst`);
    if (!response.ok) throw new Error("Server error");
    const data = await response.json();
...
const findByBudget = async (airportId: number) => {
  ...
  try {
    const response = await fetch(
      `${API_URL}/budget?id=${airportId}&maxBudget=${maxBudget}`
    );
